...
function buildShareHtml(req, entity) {
  ...
  const description = entity
    ? `${entity.rarityLabel} · drop ${formatDropChance(entity.dropChance)} · value gained: zero`
...
<html lang="en">
  ...
  <body>
    <main class="card">
      <div class="titlebar"><span>NOPEDEX DISCOVERY</span><span>$NOPE</span></div>
      ${
        entity
          ? `<div class="media"><img src="${escapeHtml(stickerImageUrl)}" alt="${escapeHtml(entity.name)}" /></div>
      <h1>${escapeHtml(entity.name)}</h1>
      <span class="badge">${escapeHtml(entity.rarityLabel)} · drop ${escapeHtml(formatDropChance(entity.dropChance))}</span>
